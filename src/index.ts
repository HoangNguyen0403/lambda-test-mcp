...
// ── Input validation ──────────────────────────────────────────────────────────
export function validateBuildId(id: unknown): string {
...
export function extractErrorBlock(log: string): string {
...
export async function runServer() {
  ...
  if (!username || !accessKey) {
    process.stderr.write(
      'Error: LT_USERNAME and LT_ACCESS_KEY environment variables are required\n',
    );
    process.exit(1);
  }
  ...
  server.tool(
    ...
    'List all failed test sessions in a LambdaTest App Automation build. Returns session IDs, test names, duration, and direct dashboard URLs. Use this first to find which sessions to debug.',
    {
      build_id: z
        .string()
        .describe('LambdaTest build ID — visible in the build URL (e.g. 19305815)'),
    },
    async ({ build_id }) => {
